...
xml.place do
	...
	xml.id @place.id
	xml.owner @place.owner
	xml.owner_email_address @place.owner_email_address
	xml.additional_info_url @place.url
	xml.payment_methods @place.payment_methods
	xml.description @place.description
	xml.menu_link @place.menu_link
	xml.photo_url "#{request.protocol}#{request.host}#{":#{request.port}" unless request.port == 80}#{@place.photo.url(:mobile)}" if @place.photo.file?
end